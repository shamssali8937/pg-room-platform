    ...
    const mappedListings = useMemo((): ModerationListing[] => {
        return reduxRooms.map((r) => {
            ...
            return {
                ...
                propertyCount: 1,
                description: r.description ?? `${r.room_type} room with ${r.beds} beds and ${r.baths} baths.`,
                amenities: r.amenities_list && r.amenities_list.length > 0 ? r.amenities_list : (r.amenities && r.amenities.length > 0 ? r.amenities : ["WiFi", "AC", "Laundry"]),
                ...
                locality: r.locality ?? "",
                landmark: r.landmark ?? "",
                furnishedStatus: r.furnished_status ?? "unfurnished",
                securityDeposit: r.security_deposit_amount ?? 0,
                availableFor: r.available_for ?? "any",
