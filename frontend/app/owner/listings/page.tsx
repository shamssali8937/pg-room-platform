...
const STATUS_CONFIG: Record<string, {
    ...
    suspended: {
        dot: "bg-rose-500",
        badge: (d) => d ? "bg-rose-500/10 text-rose-400 border border-rose-500/20" : "bg-rose-50 text-rose-700 border border-rose-200",
        icon: <AlertTriangle size={12} />,
        label: "Suspended",
    },
    draft: {
...
function roomToOwnerListing(room: Room): any {
    return {
        ...
        status: room.status === "active" ? "Live" : room.status === "pending" ? "Pending Review" : room.status === "rejected" ? "Rejected" : room.status === "suspended" ? "Suspended" : "Pending Review",
...
type FilterType = "all" | "active" | "pending" | "rejected" | "suspended";
...
export default function OwnerListingsPage() {
    ...
    const counts = {
        ...
        suspended: ownerRooms.filter((r) => r.status === "suspended").length,
    };
                        ...
                        {filtered.map((room) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`group rounded-2xl overflow-hidden transition-all duration-500 hover:scale-[1.02] ${surfaceHigh} ${(isRejected || isSuspended) ? "opacity-80 hover:opacity-100" : ""}`}
                                >
                                    {/* Image */}
                                    <div className="relative aspect-[16/10] overflow-hidden">
                                        {room.images?.[0] ? (
                                            <img
                                                ...
                                                alt={room.title}
                                                className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 ${(isRejected || isSuspended) ? "grayscale" : ""}`}
                                            />
                                        ) : (
                                        ...
                                        {isSuspended && (
                                            <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10">
                                                <span className="text-rose-400 text-xs font-bold uppercase tracking-[0.3em] rotate-12 border-4 border-rose-400 px-4 py-2">Suspended</span>
                                            </div>
                                        )}
                                    ...
                                    <div className="p-5">
                                        ...
                                        {isSuspended && room.rejection_reason ? (
                                            <div className={`mb-3 p-3 rounded-xl border text-xs text-rose-500 leading-relaxed ${isDark ? "bg-rose-500/8 border-rose-500/15" : "bg-rose-50 border-rose-200"}`}>
                                                <span className="font-bold">Suspension Reason: </span>{room.rejection_reason}
                                            </div>
