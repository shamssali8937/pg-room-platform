                        ...
                        {filtered.map((room, idx) => (
                            ...
                            >
                                    <div className="relative aspect-[16/10] overflow-hidden">
                                        <img
                                            src={room.images?.[0]?.url ?? "https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=800&q=80"}
                                            alt={room.title}
                                            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                                        />
                                        {room.status === "occupied" && (
                                            <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10 backdrop-blur-[1px]">
                                                <span className="px-4 py-2 border-2 border-rose-500 text-rose-500 font-black text-sm uppercase tracking-widest rounded-lg transform -rotate-12 shadow-lg bg-black/50">
                                                    Occupied
                                                </span>
                                            </div>
                                        )}
                                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-3 backdrop-blur-[2px] z-20">
                                        <motion.button
